...
export class SfdtReader {
  ...
  private commentStarts: Dictionary<
    string,
    CommentCharacterElementBox
  > = undefined;
  private commentEnds: Dictionary<
    string,
    CommentCharacterElementBox
  > = undefined;
  ...
  public convertJsonToDocument(json: string): BodyWidget[] {
    ...
    jsonObject =
      jsonObject instanceof Object ? jsonObject : JSON.parse(jsonObject);
    ...
    let paragraphFormat: any = isNullOrUndefined(jsonObject.paragraphFormat)
      ? this.viewer.owner.paragraphFormat
      : jsonObject.paragraphFormat;
    this.parseParagraphFormat(
      paragraphFormat,
    ...
    if (!isNullOrUndefined(jsonObject.background)) {
      this.documentHelper.backgroundColor = this.getColor(
        jsonObject.background.color
      );
    }
    ...
    if (!isNullOrUndefined(jsonObject.revisions)) {
      this.parseRevisions(
        jsonObject,
        this.viewer.owner.revisionsInternal.changes
      );
    }
  ...
  private parseStyles(data: any, styles: WStyles): void {
    for (let i: number = 0; i < data.styles.length; i++) {
      if (
        isNullOrUndefined(
          this.documentHelper.styles.findByName(data.styles[i].name)
        )
      ) {
    ...
    for (let i: number = 0; i < data.revisions.length; i++) {
      ...
      if (!isNullOrUndefined(revisionData.revisionId)) {
        ...
        if (
          ...
          if (
            revisionData.revisionType === "Insertion" &&
            this.isPaste &&
            this.documentHelper.owner.enableTrackChanges
          ) {
            continue;
          } else {
  ...
  private checkAndApplyRevision(inline: any, item: any): void {
    if (
      !isNullOrUndefined(inline.revisionIds) &&
  ...
  private parseComments(data: any, comments: CommentElementBox[]): void {
    ...
    for (let i: number = 0; i < data.comments.length; i++) {
      ...
      while (count < commentData.replyComments.length) {
        ...
        replyComment = this.parseComment(
          commentData.replyComments[count],
          replyComment
        );
  ...
  private parseComment(
    commentData: any,
    commentElement: CommentElementBox
  ): CommentElementBox {
    ...
    if (!isNullOrUndefined(style.type)) {
      ...
      if (!isNullOrUndefined(style.basedOn)) {
        ...
        if (!isNullOrUndefined(basedOn)) {
          ...
          let styleString: any;
          if (
            !isNullOrUndefined(basedStyle) &&
            basedStyle.type === wStyle.type
          ) {
            styleString = basedStyle;
          } else {
            if (wStyle.type === "Paragraph") {
              styleString = JSON.parse(
      ...
      if (!isNullOrUndefined(style.link)) {
        ...
        if (isNullOrUndefined(link)) {
          ...
          this.parseStyle(data, styleString, styles);
          wStyle.link = isNullOrUndefined(styles.findByName(styleString.name))
            ? style.link
            : styles.findByName(styleString.name);
        } else {
          wStyle.link = link;
        }
      }
      ...
      if (!isNullOrUndefined(style.next)) {
        if (style.next === style.name) {
          ...
          if (
            ...
            (next as WStyle).type === wStyle.type
          ) {
            wStyle.next = next;
          } else {
        ...
        if (!isNullOrUndefined(abstract.levels)) {
          for (let j: number = 0; j < abstract.levels.length; j++) {
            ...
            if (!isNullOrUndefined(level)) {
              let listLevel: WListLevel = this.parseListLevel(
                level,
                abstractList
              );
  ...
  private parseListLevel(data: any, owner: any): WListLevel {
    ...
    if (data.listLevelPattern === "Bullet") {
      ...
      listLevel.numberFormat = !isNullOrUndefined(data.numberFormat)
        ? data.numberFormat
        : "";
    } else {
  ...
  public parseList(data: any, listCollection: WList[]): void {
    for (let i: number = 0; i < data.lists.length; i++) {
      ...
      if (!isNullOrUndefined(lists.abstractListId)) {
        ...
        list.abstractList = this.documentHelper.getAbstractListById(
          lists.abstractListId
        );
      }
  ...
  private parseLevelOverride(data: any, list: WList): void {
    ...
    for (let i: number = 0; i < data.length; i++) {
      ...
      if (!isNullOrUndefined(levelOverride.overrideListLevel)) {
        levelOverrides.overrideListLevel = this.parseListLevel(
          levelOverride.overrideListLevel,
          levelOverrides
        );
      }
  ...
  private parseSections(data: any, sections: BodyWidget[]): void {
    for (let i: number = 0; i < data.length; i++) {
      ...
      this.documentHelper.headersFooters.push(
        this.parseHeaderFooter(
          item.headersFooters,
  ...
  public parseHeaderFooter(data: any, headersFooters: any): HeaderFooters {
    ...
    if (!isNullOrUndefined(data.firstPageHeader)) {
      let firstPageHeader: HeaderFooterWidget = new HeaderFooterWidget(
        "FirstPageHeader"
      );
    ...
    if (!isNullOrUndefined(data.firstPageFooter)) {
      let firstPageFooter: HeaderFooterWidget = new HeaderFooterWidget(
        "FirstPageFooter"
      );
  ...
  private parseTextBody(
    ...
    this.parseBody(
      data,
      section.childWidgets as BlockWidget[],
      section,
      isSectionBreak
    );
  }
  public addCustomStyles(data: any): void {
    for (let i: number = 0; i < data.styles.length; i++) {
      let style: any = this.documentHelper.styles.findByName(
        data.styles[i].name
      );
      if (style === undefined) {
      ...
      for (let i: number = 0; i < data.length; i++) {
        ...
        if (block.hasOwnProperty("inlines")) {
          ...
          if (block.inlines.length > 0) {
            hasValidElmts = this.parseParagraph(
              block.inlines,
              paragraph,
              writeInlineFormat
            );
          ...
          if (
            ...
            let styleObj: Object;
            if (
              !isNullOrUndefined(block.paragraphFormat) &&
              !isNullOrUndefined(block.paragraphFormat.styleName)
            ) {
              styleObj = this.documentHelper.styles.findByName(
          ...
          let blockStartContentControl: ContentControl = new ContentControl(
            "Block"
          );
          let blockEndContentControl: ContentControl = new ContentControl(
            "Block"
          );
          this.parseContentControlProperties(
            block.contentControlProperties,
            blockStartContentControl.contentControlProperties
          );
          blockEndContentControl.contentControlProperties =
            blockStartContentControl.contentControlProperties;
          ...
          for (let j: number = 0; j < 2; j++) {
            let para: IWidget =
              j === 0
                ? blocks[blocks.length - block.blocks.length]
            ...
            if (para instanceof ParagraphWidget) {
              ...
              if (j === 0) {
                blockWidget = ((para.firstChild as TableRowWidget)
                  .firstChild as TableCellWidget).firstChild as BlockWidget;
              } else {
                let cell: TableCellWidget = (para.lastChild as TableRowWidget)
                  .lastChild as TableCellWidget;
                blockWidget = cell.lastChild as BlockWidget;
              }
            }
            if (blockWidget.childWidgets.length === 0) {
              let lineWidget: LineWidget = new LineWidget(
                blockWidget as ParagraphWidget
              );
              blockWidget.childWidgets.push(lineWidget);
            }
            if (j === 0) {
              (blockWidget.firstChild as LineWidget).children.splice(
                0,
                0,
                blockStartContentControl
              );
              blockStartContentControl.line = blockWidget.firstChild as LineWidget;
            } else {
              (blockWidget.lastChild as LineWidget).children.push(
                blockEndContentControl
              );
        ...
        if (!isNullOrUndefined(contentControlProperties)) {
          (blocks[
            blocks.length - 1
          ] as BlockWidget).contentControlProperties = contentControlProperties;
        }
      }
    }
  }
  private parseTable(
    ...
    for (let i: number = 0; i < block.rows.length; i++) {
      ...
      if (!isNullOrUndefined(tableRow.contentControlProperties)) {
        ...
        this.parseContentControlProperties(
          tableRow.contentControlProperties,
          row.contentControlProperties
        );
      }
      if (tableRow.hasOwnProperty("rowFormat")) {
        ...
        for (let j: number = 0; j < block.rows[i].cells.length; j++) {
          ...
          if (
            !isNullOrUndefined(block.rows[i].cells[j].contentControlProperties)
          ) {
            cell.contentControlProperties = new ContentControlProperties(
              "Cell"
            );
            this.parseContentControlProperties(
              block.rows[i].cells[j].contentControlProperties,
              cell.contentControlProperties
            );
          }
          ...
          if (block.rows[i].cells[j].hasOwnProperty("cellFormat")) {
            this.parseCellFormat(
              block.rows[i].cells[j].cellFormat,
              cell.cellFormat
            );
          }
          ...
          if (!isNullOrUndefined(cell.contentControlProperties)) {
            let cellStartContentControl: ContentControl = new ContentControl(
              "Cell"
            );
            let cellEndContentControl: ContentControl = new ContentControl(
              "Cell"
            );
            cellStartContentControl.contentControlProperties =
              cell.contentControlProperties;
            cellEndContentControl.contentControlProperties =
              cell.contentControlProperties;
            cellStartContentControl.type = 0;
            cellEndContentControl.type = 1;

            if (
              (cell.firstChild as ParagraphWidget).childWidgets.length === 0
            ) {
              let lineWidget: LineWidget = new LineWidget(
                cell.firstChild as ParagraphWidget
              );
              (cell.firstChild as ParagraphWidget).childWidgets.push(
                lineWidget
              );
            }
            cellStartContentControl.line = (cell.firstChild as ParagraphWidget)
              .firstChild as LineWidget;
            ((cell.firstChild as ParagraphWidget)
              .firstChild as LineWidget).children.splice(
              0,
              0,
              cellStartContentControl
            );
            cellEndContentControl.line = (cell.lastChild as ParagraphWidget)
              .lastChild as LineWidget;
            ((cell.lastChild as ParagraphWidget)
              .lastChild as LineWidget).children.push(cellEndContentControl);
          }
          if (!isNullOrUndefined(row.contentControlProperties)) {
            if (row.firstChild === cell) {
              let rowStartContentControl: ContentControl = new ContentControl(
                "Row"
              );
              rowStartContentControl.contentControlProperties =
                row.contentControlProperties;
              ...
              rowStartContentControl.line = (cell.firstChild as ParagraphWidget)
                .firstChild as LineWidget;
              ((cell.firstChild as ParagraphWidget)
                .firstChild as LineWidget).children.splice(
                0,
                0,
                rowStartContentControl
              );
            } else if (row.lastChild === cell) {
              let rowEndContentControl: ContentControl = new ContentControl(
                "Row"
              );
              rowEndContentControl.contentControlProperties =
                row.contentControlProperties;
              rowEndContentControl.type = 1;
              if (
                (cell.lastChild as ParagraphWidget).childWidgets.length === 0
              ) {
                let lineWidget: LineWidget = new LineWidget(
                  cell.lastChild as ParagraphWidget
                );
                (cell.lastChild as ParagraphWidget).childWidgets.push(
                  lineWidget
                );
              }
              rowEndContentControl.line = (cell.lastChild as ParagraphWidget)
                .lastChild as LineWidget;
              ((cell.lastChild as ParagraphWidget)
                .lastChild as LineWidget).children.push(rowEndContentControl);
            }
          }
  ...
  private parseContentControlProperties(
    ...
    if (!isNullOrUndefined(wContentControlProperties.appearance)) {
      contentControlProperties.appearance =
        wContentControlProperties.appearance;
    }
    ...
    if (!isNullOrUndefined(wContentControlProperties.hasPlaceHolderText)) {
      contentControlProperties.hasPlaceHolderText =
        wContentControlProperties.hasPlaceHolderText;
    }
    ...
    if (!isNullOrUndefined(wContentControlProperties.isTemporary)) {
      contentControlProperties.isTemporary =
        wContentControlProperties.isTemporary;
    }
    if (!isNullOrUndefined(wContentControlProperties.characterFormat)) {
      this.parseCharacterFormat(
        wContentControlProperties.characterFormat,
        contentControlProperties.characterFormat
      );
    }
    if (contentControlProperties.type === "CheckBox") {
      if (!isNullOrUndefined(wContentControlProperties.isChecked)) {
        contentControlProperties.isChecked =
          wContentControlProperties.isChecked;
      }
      if (!isNullOrUndefined(wContentControlProperties.uncheckedState)) {
        contentControlProperties.uncheckedState = new CheckBoxState();
        contentControlProperties.uncheckedState.font =
          wContentControlProperties.uncheckedState.font;
        contentControlProperties.uncheckedState.value =
          wContentControlProperties.uncheckedState.value;
      }
      if (!isNullOrUndefined(wContentControlProperties.checkedState)) {
        contentControlProperties.checkedState = new CheckBoxState();
        contentControlProperties.checkedState.font =
          wContentControlProperties.checkedState.font;
        contentControlProperties.checkedState.value =
          wContentControlProperties.checkedState.value;
      }
    } else if (contentControlProperties.type === "Date") {
      if (!isNullOrUndefined(wContentControlProperties.dateCalendarType)) {
        contentControlProperties.dateCalendarType =
          wContentControlProperties.dateCalendarType;
      }
      if (!isNullOrUndefined(wContentControlProperties.dateStorageFormat)) {
        contentControlProperties.dateStorageFormat =
          wContentControlProperties.dateStorageFormat;
      }
      if (!isNullOrUndefined(wContentControlProperties.dateDisplayLocale)) {
        contentControlProperties.dateDisplayLocale =
          wContentControlProperties.dateDisplayLocale;
      }
      if (!isNullOrUndefined(wContentControlProperties.dateDisplayFormat)) {
        contentControlProperties.dateDisplayFormat =
          wContentControlProperties.dateDisplayFormat;
      }
    } else if (
      contentControlProperties.type === "ComboBox" ||
      contentControlProperties.type === "DropDownList"
    ) {
      if (
        !isNullOrUndefined(wContentControlProperties.contentControlListItems)
      ) {
        for (
          let i: number = 0;
          i < wContentControlProperties.contentControlListItems.length;
          i++
        ) {
          let contentControlListItem: ContentControlListItems = new ContentControlListItems();
          contentControlListItem.displayText =
            wContentControlProperties.contentControlListItems[i].displayText;
          contentControlListItem.value =
            wContentControlProperties.contentControlListItems[i].value;
          contentControlProperties.contentControlListItems.push(
    ...
    if (!isNullOrUndefined(wContentControlProperties.xmlMapping)) {
      ...
      contentControlProperties.xmlMapping.isMapped =
        wContentControlProperties.xmlMapping.isMapped;
      contentControlProperties.xmlMapping.isWordMl =
        wContentControlProperties.xmlMapping.isWordMl;
      if (
        !isNullOrUndefined(wContentControlProperties.xmlMapping.prefixMapping)
      ) {
        contentControlProperties.xmlMapping.prefixMapping =
          wContentControlProperties.xmlMapping.prefixMapping;
      }
      contentControlProperties.xmlMapping.xPath =
        wContentControlProperties.xmlMapping.xPath;
      contentControlProperties.xmlMapping.storeItemId =
        wContentControlProperties.xmlMapping.storeItemId;
      if (
        !isNullOrUndefined(wContentControlProperties.xmlMapping.customXmlPart)
      ) {
        contentControlProperties.xmlMapping.customXmlPart = new CustomXmlPart();
        contentControlProperties.xmlMapping.customXmlPart.id =
          wContentControlProperties.xmlMapping.customXmlPart.id;
        contentControlProperties.xmlMapping.customXmlPart.xml =
          wContentControlProperties.xmlMapping.customXmlPart.xml;
      }
    }
  }
    ...
    for (let i: number = 0; i < data.length; i++) {
      ...
      if (inline.hasOwnProperty("text")) {
        let textElement:
          | FieldTextElementBox
          | TextElementBox
          | TabElementBox = undefined;
        if (this.documentHelper.isPageField) {
          textElement = new FieldTextElementBox();
          // tslint:disable-next-line:max-line-length
          (textElement as FieldTextElementBox).fieldBegin = this.documentHelper.fieldStacks[
            this.documentHelper.fieldStacks.length - 1
          ];
        } else if (inline.text === "\t") {
          ...
          if (!isNullOrUndefined(inline.revisionIds)) {
            for (let j: number = 0; j < inline.revisionIds.length; j++) {
              if (this.revisionCollection.containsKey(inline.revisionIds[j])) {
                ...
                // tslint:disable-next-line:max-line-length
                if (
                  !isNullOrUndefined(revision) &&
                  !isNullOrUndefined(lineWidget.children[i - 1].revisions[j]) &&
                  (!trackChange ||
                    (trackChange && revision.revisionType === "Deletion"))
                ) {
                  if (revision.revisionID === inline.revisionIds[j]) {
                    inline.revisionIds[j] =
                      lineWidget.children[i - 1].revisions[j].revisionID;
                    this.checkAndApplyRevision(inline, textElement);
                    continue;
                  }
                }
                if (!trackChange) {
                  revision = this.documentHelper.revisionsInternal.get(
                    inline.revisionIds[j]
                  );
                }
                this.documentHelper.owner.editorModule.insertRevision(
                  textElement,
        ...
        this.parseChartTitleArea(
          inline.chartTitleArea,
          chartElement.chartTitleArea
        );
        ...
        this.parseChartCategoryAxis(
          inline.chartPrimaryCategoryAxis,
          chartElement.chartPrimaryCategoryAxis
        );
        this.parseChartCategoryAxis(
          inline.chartPrimaryValueAxis,
          chartElement.chartPrimaryValueAxis
        );
        if (inline.chartDataTable != null) {
          this.parseChartDataTable(
            inline.chartDataTable,
            chartElement.chartDataTable
          );
        }
        ...
        let imageString: string = HelperMethods.formatClippedString(
          inline.imageString
        ).formatClippedString;
        ...
        this.parseCharacterFormat(
          inline.characterFormat,
          image.characterFormat
        );
        hasValidElmts = true;
      } else if (
        inline.hasOwnProperty("hasFieldEnd") ||
        (inline.hasOwnProperty("fieldType") && inline.fieldType === 0)
      ) {
        let fieldBegin: FieldElementBox = new FieldElementBox(0);
        this.parseCharacterFormat(
          inline.characterFormat,
          fieldBegin.characterFormat,
          writeInlineFormat
        );
        ...
        if (inline.hasOwnProperty("formFieldData")) {
          ...
          if (inline.formFieldData.hasOwnProperty("textInput")) {
            ...
            (formFieldData as TextFormField).type =
              inline.formFieldData.textInput.type;
            (formFieldData as TextFormField).maxLength =
              inline.formFieldData.textInput.maxLength;
            (formFieldData as TextFormField).defaultValue =
              inline.formFieldData.textInput.defaultValue;
            (formFieldData as TextFormField).format =
              inline.formFieldData.textInput.format;
          } else if (inline.formFieldData.hasOwnProperty("checkBox")) {
            formFieldData = new CheckBoxFormField();
            (formFieldData as CheckBoxFormField).sizeType =
              inline.formFieldData.checkBox.sizeType;
            (formFieldData as CheckBoxFormField).size =
              inline.formFieldData.checkBox.size;
            (formFieldData as CheckBoxFormField).defaultValue =
              inline.formFieldData.checkBox.defaultValue;
            (formFieldData as CheckBoxFormField).checked =
              inline.formFieldData.checkBox.checked;
          } else {
            formFieldData = new DropDownFormField();
            (formFieldData as DropDownFormField).dropdownItems =
              inline.formFieldData.dropDownList.dropDownItems;
            (formFieldData as DropDownFormField).selectedIndex =
              inline.formFieldData.dropDownList.selectedIndex;
          }
        ...
        if (inline.fieldType === 2) {
          ...
          if (this.documentHelper.fieldStacks.length > 0) {
            field.fieldBegin = this.documentHelper.fieldStacks[
              this.documentHelper.fieldStacks.length - 1
            ];
            ...
            if (
              lineWidgetCount >= 2 &&
              lineWidget.children[lineWidgetCount - 2] instanceof
                FieldElementBox &&
              (lineWidget.children[lineWidgetCount - 2] as FieldElementBox)
                .hasFieldEnd &&
              lineWidget.children[lineWidgetCount - 1] instanceof TextElementBox
            ) {
              let fieldElementText: string = (lineWidget.children[
                lineWidgetCount - 1
              ] as TextElementBox).text;
          ...
          if (
            !isNullOrUndefined(field.fieldBegin) &&
            field.fieldBegin.fieldSeparator
          ) {
          ...
          this.documentHelper.fieldStacks.splice(
            this.documentHelper.fieldStacks.length - 1,
            1
          );
        ...
        if (!this.isParseHeader || this.isPaste) {
          if (inline.bookmarkType === 0) {
            let isAdd: boolean =
              this.isPaste &&
              !this.documentHelper.bookmarks.containsKey(bookmark.name);
            if (!this.isPaste || isAdd) {
              this.documentHelper.bookmarks.add(bookmark.name, bookmark);
            } else if (!isAdd) {
              lineWidget.children.splice(
                lineWidget.children.indexOf(bookmark),
                1
              );
            }
          } else if (inline.bookmarkType === 1) {
            if (this.documentHelper.bookmarks.containsKey(bookmark.name)) {
              let bookmarkStart: BookmarkElementBox = this.documentHelper.bookmarks.get(
                bookmark.name
              );
              let isConsider: boolean =
                this.isPaste && isNullOrUndefined(bookmarkStart.reference);
        ...
        if (inline.hasOwnProperty("editableRangeStart")) {
          ...
          if (this.editableRanges.containsKey(inline.editRangeId)) {
            let start: EditRangeStartElementBox = this.editableRanges.get(
              inline.editRangeId
            );
          ...
          let permStart: EditRangeStartElementBox = this.parseEditableRangeStart(
            inline
          );
        ...
        if (inline.hasOwnProperty("commentCharacterType")) {
          if (inline.commentCharacterType === 0) {
            let commentStartElement: CommentCharacterElementBox = new CommentCharacterElementBox(
              0
            );
            ...
            let comment: CommentElementBox = this.commentsCollection.get(
              commentID
            );
            ...
            let commentEndElement: CommentCharacterElementBox = new CommentCharacterElementBox(
              1
            );
        ...
        shape.verticalPosition = HelperMethods.convertPointToPixel(
          inline.verticalPosition
        );
        shape.verticalOrigin = inline.verticalOrigin;
        shape.verticalAlignment = inline.verticalAlignment;
        shape.horizontalPosition = HelperMethods.convertPointToPixel(
        ...
        if (inline.hasOwnProperty("textFrame")) {
          ...
          textFrame.textVerticalAlignment =
            inline.textFrame.textVerticalAlignment;
          ...
          this.parseBody(
            ...
            textFrame.childWidgets as BlockWidget[],
            textFrame
          );
        ...
        let inlineStartContentControl: ContentControl = new ContentControl(
          "Inline"
        );
        let inlineEndContentControl: ContentControl = new ContentControl(
          "Inline"
        );
        this.parseContentControlProperties(
          inline.contentControlProperties,
          inlineStartContentControl.contentControlProperties
        );
        inlineEndContentControl.contentControlProperties =
          inlineStartContentControl.contentControlProperties;
        ...
        let element: ElementBox =
          lineWidget.children[lineWidget.children.length - 1];
        while (!(element instanceof ContentControl)) {
          (element as ElementBox).contentControlProperties =
            inlineStartContentControl.contentControlProperties;
  ...
  private applyCharacterStyle(inline: any, elementbox: ElementBox): void {
    /* tslint:disable-next-line:max-line-length */
    if (
      !isNullOrUndefined(inline.characterFormat) &&
      !isNullOrUndefined(inline.characterFormat.styleName)
    ) {
      let charStyle: Object = this.documentHelper.styles.findByName(
        inline.characterFormat.styleName,
        "Character"
      );
  ...
  private parseEditableRangeStart(data: any): EditRangeStartElementBox {
    ...
    if (!isNullOrUndefined(data.group)) {
      ...
      permStart.group =
        permStart.group === "everyone" ? "Everyone" : permStart.group;
  ...
  private addEditRangeCollection(
    name: string,
    permStart: EditRangeStartElementBox
  ): void {
    if (this.documentHelper.editRanges.containsKey(name)) {
      let editStartCollection: EditRangeStartElementBox[] = this.documentHelper.editRanges.get(
        name
      );
  ...
  private parseTableFormat(sourceFormat: any, tableFormat: WTableFormat): void {
    ...
    if (!isNullOrUndefined(sourceFormat.subSeccion)) {
      tableFormat.subSeccion = sourceFormat.subSeccion;
    }
  ...
  private parseRowFormat(sourceFormat: any, rowFormat: WRowFormat): void {
    if (!isNullOrUndefined(sourceFormat)) {
      ...
      if (
        !isNullOrUndefined(sourceFormat.revisionIds) &&
        sourceFormat.revisionIds.length > 0
      ) {
  ...
  private parseShading(sourceShading: any, destShading: WShading): void {
    if (!isNullOrUndefined(sourceShading)) {
      if (!isNullOrUndefined(sourceShading.backgroundColor)) {
        destShading.backgroundColor = this.getColor(
          sourceShading.backgroundColor
        );
      }
      if (!isNullOrUndefined(sourceShading.foregroundColor)) {
        destShading.foregroundColor = this.getColor(
          sourceShading.foregroundColor
        );
      }
      if (
        !isNullOrUndefined(sourceShading.texture) ||
        !isNullOrUndefined(sourceShading.textureStyle)
      ) {
        destShading.textureStyle = !isNullOrUndefined(sourceShading.texture)
          ? sourceShading.texture
          : sourceShading.textureStyle;
      }
    }
  }
  /**
  ...
  public parseCharacterFormat(
    ...
    if (!isNullOrUndefined(sourceFormat)) {
      ...
      if (!isNullOrUndefined(sourceFormat.configSubseccion)) {
        characterFormat.configSubseccion = sourceFormat.configSubseccion;
      }
  ...
  private parseListFormat(block: any, listFormat: WListFormat): void {
    if (!isNullOrUndefined(block.listFormat)) {
      if (!isNullOrUndefined(block.listFormat.listId)) {
        ...
        listFormat.list = this.documentHelper.getListById(
          block.listFormat.listId
        );
      }
  ...
  public parseSectionFormat(data: any, sectionFormat: WSectionFormat): void {
    if (!isNullOrUndefined(data.seccion)) {
      sectionFormat.seccion = data.seccion;
    }
