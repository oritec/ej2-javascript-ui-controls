...
export class HelperMethods {
   ...
   * @private
   */
  public static lastIndexOfAny(
    text: string,
    wordSplitCharacter: string[]
  ): number {
  ...
  public static isLinkedFieldCharacter(inline: ElementBox): boolean {
    if (
      inline instanceof FieldElementBox &&
      (inline as FieldElementBox).fieldType === 0
    ) {
      return !isNullOrUndefined(inline.fieldEnd);
    } else if (
      inline instanceof FieldElementBox &&
      (inline as FieldElementBox).fieldType === 2
    ) {
      return (
        !isNullOrUndefined(inline.fieldBegin) &&
   ...
   */
  public static writeCharacterFormat(
    characterFormat: any,
    isInline: boolean,
    format: WCharacterFormat
  ): void {
    characterFormat.configSubseccion = format.getValue("configSubseccion");
    ...
    characterFormat.fontSize = isInline
      ? this.toWriteInline(format, "fontSize")
      : format.getValue("fontSize");
    characterFormat.fontFamily = isInline
      ? this.toWriteInline(format, "fontFamily")
    ...
    characterFormat.fontColor = isInline
      ? this.toWriteInline(format, "fontColor")
      : format.getValue("fontColor");
    characterFormat.styleName = !isNullOrUndefined(format.baseCharStyle)
    ...
    characterFormat.allCaps = isInline
      ? format.allCaps
      : format.getValue("allCaps");
  }
  public static toWriteInline(
    format: WCharacterFormat,
    propertyName: string
  ): any {
    if (
      !isNullOrUndefined(format.ownerBase) &&
      format.ownerBase instanceof ElementBox
    ) {
      return format.hasValue(propertyName)
        ? format[propertyName]
        : format.getValue(propertyName);
    } else {
  ...
  public static capitaliseFirst(
    ...
    if (type === "Titlecase") {
      ...
      for (let i: number = 0; i < valArry.length; i++) {
        ...
        text += splitBy
          ? valArry[i].charAt(0).toUpperCase() +
            valArry[i].slice(1, valArry[i].length)
          : this.capitaliseFirstInternal(valArry[i]);
  ...
  public static getModifiedDate(date: string): string {
    ...
    let dateString: string = modifiedDate.toLocaleDateString([], {
      year: "numeric",
      month: "long",
      day: "numeric"
    });
    let time: string = modifiedDate.toLocaleTimeString([], {
      hour: "numeric",
      minute: "numeric"
    });
