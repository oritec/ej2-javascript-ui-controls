...
export class SfdtExport {
  ...
  private updateEditRangeId(): void {
    ...
    for (
      ...
      for (let j: number = 0; j < keys[i].length; j++) {
        let editRangeStart: EditRangeStartElementBox[] = this.documentHelper.editRanges.get(
          keys[i]
        );
  ...
  public Initialize(): void {
    ...
    this.document.characterFormat = this.writeCharacterFormat(
      this.documentHelper.characterFormat
    );
    this.document.paragraphFormat = this.writeParagraphFormat(
  ...
  private createSection(bodyWidget: BlockContainer): any {
    ...
    section.sectionFormat.seccion = bodyWidget.sectionFormat.seccion;
    ...
    section.sectionFormat.differentFirstPage =
      bodyWidget.sectionFormat.differentFirstPage;
    section.sectionFormat.differentOddAndEvenPages =
      bodyWidget.sectionFormat.differentOddAndEvenPages;
    section.sectionFormat.headerDistance =
      bodyWidget.sectionFormat.headerDistance;
    section.sectionFormat.footerDistance =
    ...
    if (
      ...
      if (
        ...
        (this.blockContent &&
          firstChild instanceof ContentControl &&
          !this.nestedBlockContent &&
          (firstChild as ContentControl).type === 0 &&
          (firstChild as ContentControl).contentControlWidgetType === "Block")
      ) {
  ...
  private contentControlProperty(
    ...
    if (!isNullOrUndefined(contentControlPropertie.uncheckedState)) {
      contentControlProperties.uncheckedState = this.tounCheckedState(
        contentControlPropertie.uncheckedState
      );
    }
    if (!isNullOrUndefined(contentControlPropertie.checkedState)) {
      contentControlProperties.checkedState = this.toCheckedState(
        contentControlPropertie.checkedState
      );
    }
    ...
    if (!isNullOrUndefined(contentControlPropertie.characterFormat)) {
      contentControlProperties.characterFormat = this.writeCharacterFormat(
        contentControlPropertie.characterFormat
      );
    }
    contentControlProperties.contentControlListItems =
      contentControlPropertie.contentControlListItems;
  ...
  private blockContentControl(widget: ParagraphWidget): any {
    ...
    if (!isNullOrUndefined(block.blocks)) {
      ...
      if (
        ...
        if (!(secondChild instanceof ContentControl)) {
          block.contentControlProperties = this.contentControlProperty(
            firstChild.contentControlProperties
          );
          return block;
        } else if (
          (secondChild as ContentControl).contentControlWidgetType === "Block"
          ...
          block.contentControlProperties = this.contentControlProperty(
            secondChild.contentControlProperties
          );
        } else {
          block.contentControlProperties = this.contentControlProperty(
  ...
  private inlineContentControl(
    ...
    if (!isNullOrUndefined(inlines)) {
      ...
      if (this.nestedContent) {
        inline.inlines = this.inlineContentControls(
          element,
          undefined,
          nestedContentInline
        );
    ...
    if (!isNullOrUndefined(nextElement)) {
      if (nextElement.type === 1 && !this.nestedContent) {
        if (this.multipleLineContent) {
          ...
          inlines[
            inlines.length - 1
          ].contentControlProperties = this.contentControlProperty(
            nextElement.contentControlProperties
          );
          this.multipleLineContent = false;
          return;
        } else {
          inline.contentControlProperties = this.contentControlProperty(
            nextElement.contentControlProperties
          );
        }
        return inline;
  ...
  private writeShape(element: ShapeElementBox, inline: any): void {
    ...
    if (element.textFrame) {
      ...
      inline.textFrame.leftMargin = HelperMethods.convertPixelToPoint(
        element.textFrame.marginLeft
      );
      inline.textFrame.rightMargin = HelperMethods.convertPixelToPoint(
        element.textFrame.marginRight
      );
      inline.textFrame.topMargin = HelperMethods.convertPixelToPoint(
        element.textFrame.marginTop
      );
      inline.textFrame.bottomMargin = HelperMethods.convertPixelToPoint(
        element.textFrame.marginBottom
      );
  ...
  private createParagraph(paragraphWidget: ParagraphWidget): any {
    ...
    // tslint:disable-next-line:max-line-length
    paragraph.paragraphFormat = this.writeParagraphFormat(
      isParaSelected
        ? paragraphWidget.paragraphFormat
        : new WParagraphFormat(paragraphWidget)
    );
    paragraph.characterFormat = this.writeCharacterFormat(
      isParaSelected
  ...
  private writeParagraphFormat(
    ...
    paragraphFormat.leftIndent = isInline
      ? format.leftIndent
      : format.getValue("leftIndent");
    paragraphFormat.rightIndent = isInline
      ? format.rightIndent
      : format.getValue("rightIndent");
    paragraphFormat.firstLineIndent = isInline
      ? format.firstLineIndent
      : format.getValue("firstLineIndent");
    paragraphFormat.textAlignment = isInline
      ? format.textAlignment
      : format.getValue("textAlignment");
    paragraphFormat.beforeSpacing = isInline
      ? format.beforeSpacing
      : format.getValue("beforeSpacing");
    paragraphFormat.afterSpacing = isInline
      ? format.afterSpacing
      : format.getValue("afterSpacing");
    paragraphFormat.lineSpacing = isInline
      ? format.lineSpacing
      : format.getValue("lineSpacing");
    paragraphFormat.lineSpacingType = isInline
      ? format.lineSpacingType
      : format.getValue("lineSpacingType");
    paragraphFormat.styleName = !isNullOrUndefined(format.baseStyle)
      ? format.baseStyle.name
      : undefined;
    paragraphFormat.outlineLevel = isInline
      ? format.outlineLevel
      : format.getValue("outlineLevel");
    paragraphFormat.listFormat = this.writeListFormat(
      format.listFormat,
      isInline
    );
    paragraphFormat.tabs = this.writeTabs(format.tabs);
    paragraphFormat.bidi = isInline ? format.bidi : format.getValue("bidi");
    paragraphFormat.contextualSpacing = isInline
      ? format.contextualSpacing
      : format.getValue("contextualSpacing");
    if (this.writeInlineStyles && !isInline) {
  ...
  private createRow(rowWidget: TableRowWidget): any {
    ...
    if (rowWidget.hasOwnProperty("contentControlProperties")) {
      row.contentControlProperties = this.contentControlProperty(
        rowWidget.contentControlProperties
      );
    }
    return row;
  }
  private createCell(cellWidget: TableCellWidget): any {
    ...
    if (cellWidget.hasOwnProperty("contentControlProperties")) {
      cell.contentControlProperties = this.contentControlProperty(
        cellWidget.contentControlProperties
      );
    }
    return cell;
  }
  private writeShading(wShading: WShading): any {
    let shading: any = {};
    shading.backgroundColor = wShading.hasValue("backgroundColor")
      ? wShading.backgroundColor
      : undefined;
    shading.foregroundColor = wShading.hasValue("foregroundColor")
      ? wShading.foregroundColor
      : undefined;
    shading.textureStyle = wShading.hasValue("textureStyle")
  ...
  private writeBorder(wBorder: WBorder): any {
    ...
    border.hasNoneStyle = wBorder.hasValue("hasNoneStyle")
      ? wBorder.hasNoneStyle
      : undefined;
    border.lineStyle = wBorder.hasValue("lineStyle")
      ? wBorder.lineStyle
      : undefined;
    border.lineWidth = wBorder.hasValue("lineWidth")
      ? wBorder.lineWidth
      : undefined;
  ...
  private writeCellFormat(wCellFormat: WCellFormat): any {
    ...
    cellFormat.topMargin = wCellFormat.hasValue("topMargin")
      ? wCellFormat.topMargin
      : undefined;
    cellFormat.rightMargin = wCellFormat.hasValue("rightMargin")
      ? wCellFormat.rightMargin
      : undefined;
    cellFormat.leftMargin = wCellFormat.hasValue("leftMargin")
      ? wCellFormat.leftMargin
      : undefined;
    cellFormat.bottomMargin = wCellFormat.hasValue("bottomMargin")
      ? wCellFormat.bottomMargin
      : undefined;
    cellFormat.preferredWidth = wCellFormat.hasValue("preferredWidth")
      ? wCellFormat.preferredWidth
      : undefined;
    cellFormat.preferredWidthType = wCellFormat.hasValue("preferredWidthType")
      ? wCellFormat.preferredWidthType
      : undefined;
    cellFormat.cellWidth = wCellFormat.hasValue("cellWidth")
      ? wCellFormat.cellWidth
      : undefined;
    cellFormat.columnSpan = wCellFormat.columnSpan;
    cellFormat.rowSpan = wCellFormat.rowSpan;
    cellFormat.verticalAlignment = wCellFormat.hasValue("verticalAlignment")
  ...
  private writeRowFormat(wRowFormat: WRowFormat): any {
    let rowFormat: any = {};
    let revisionIds: any = [];
    rowFormat.height = wRowFormat.hasValue("height")
      ? wRowFormat.height
      : undefined;
    rowFormat.allowBreakAcrossPages = wRowFormat.hasValue(
      "allowBreakAcrossPages"
    )
      ? wRowFormat.allowBreakAcrossPages
      : undefined;
    rowFormat.heightType = wRowFormat.hasValue("heightType")
      ? wRowFormat.heightType
      : undefined;
    rowFormat.isHeader = wRowFormat.hasValue("isHeader")
      ? wRowFormat.isHeader
      : undefined;
    rowFormat.borders = this.writeBorders(wRowFormat.borders);
    rowFormat.gridBefore = wRowFormat.gridBefore;
    rowFormat.gridBeforeWidth = wRowFormat.hasValue("gridBeforeWidth")
      ? wRowFormat.gridBeforeWidth
      : undefined;
    rowFormat.gridBeforeWidthType = wRowFormat.hasValue("gridBeforeWidthType")
      ? wRowFormat.gridBeforeWidthType
      : undefined;
    rowFormat.gridAfter = wRowFormat.gridAfter;
    rowFormat.gridAfterWidth = wRowFormat.hasValue("gridAfterWidth")
      ? wRowFormat.gridAfterWidth
      : undefined;
    rowFormat.gridAfterWidthType = wRowFormat.hasValue("gridAfterWidthType")
      ? wRowFormat.gridAfterWidthType
      : undefined;
    rowFormat.leftMargin = wRowFormat.hasValue("leftMargin")
      ? wRowFormat.leftMargin
      : undefined;
    rowFormat.topMargin = wRowFormat.hasValue("topMargin")
      ? wRowFormat.topMargin
      : undefined;
    rowFormat.rightMargin = wRowFormat.hasValue("rightMargin")
      ? wRowFormat.rightMargin
      : undefined;
    rowFormat.bottomMargin = wRowFormat.hasValue("bottomMargin")
      ? wRowFormat.bottomMargin
      : undefined;
    rowFormat.leftIndent = wRowFormat.hasValue("leftIndent")
      ? wRowFormat.leftIndent
      : undefined;
    for (let j: number = 0; j < wRowFormat.revisions.length; j++) {
      rowFormat.revisionIds = this.writeRowRevisions(
  ...
  private writeTableFormat(wTableFormat: WTableFormat): any {
    ...
    tableFormat.subSeccion = wTableFormat.hasValue("subSeccion")
      ? wTableFormat.subSeccion
      : undefined;
    tableFormat.cellSpacing = wTableFormat.hasValue("cellSpacing")
      ? wTableFormat.cellSpacing
      : undefined;
    tableFormat.leftIndent = wTableFormat.hasValue("leftIndent")
      ? wTableFormat.leftIndent
      : undefined;
    tableFormat.tableAlignment = wTableFormat.hasValue("tableAlignment")
      ? wTableFormat.tableAlignment
      : undefined;
    tableFormat.topMargin = wTableFormat.hasValue("topMargin")
      ? wTableFormat.topMargin
      : undefined;
    tableFormat.rightMargin = wTableFormat.hasValue("rightMargin")
      ? wTableFormat.rightMargin
      : undefined;
    tableFormat.leftMargin = wTableFormat.hasValue("leftMargin")
      ? wTableFormat.leftMargin
      : undefined;
    tableFormat.bottomMargin = wTableFormat.hasValue("bottomMargin")
      ? wTableFormat.bottomMargin
      : undefined;
    tableFormat.preferredWidth = wTableFormat.hasValue("preferredWidth")
      ? wTableFormat.preferredWidth
      : undefined;
    tableFormat.preferredWidthType = wTableFormat.hasValue("preferredWidthType")
      ? wTableFormat.preferredWidthType
      : undefined;
    tableFormat.bidi = wTableFormat.hasValue("bidi")
      ? wTableFormat.bidi
      : undefined;
    tableFormat.allowAutoFit = wTableFormat.hasValue("allowAutoFit")
      ? wTableFormat.allowAutoFit
      : undefined;
    return tableFormat;
  }
  ...
  private writeStyle(style: WStyle): any {
    ...
    if (style.type === "Paragraph") {
      ...
      wStyle.paragraphFormat = this.writeParagraphFormat(
        (style as any).paragraphFormat
      );
      wStyle.characterFormat = this.writeCharacterFormat(
  ...
  public writeComments(documentHelper: DocumentHelper): void {
    ...
    for (let i: number = 0; i < documentHelper.comments.length; i++) {
      this.document.comments.push(
        this.writeComment(documentHelper.comments[i])
      );
    }
  }
  ...
  private writeLevelOverrides(wlevel: WLevelOverride): any {
    ...
    if (wlevel.overrideListLevel) {
      levelOverrides.overrideListLevel = this.writeListLevel(
        wlevel.overrideListLevel
      );
    }
  ...
  private writeListLevel(wListLevel: WListLevel): any {
    ...
    listLevel.characterFormat = this.writeCharacterFormat(
      wListLevel.characterFormat
    );
    listLevel.paragraphFormat = this.writeParagraphFormat(
      wListLevel.paragraphFormat
    );
