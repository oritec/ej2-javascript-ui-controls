...
export class WTableFormat {
  ...
  get subSeccion(): number {
    return this.getPropertyValue("subSeccion") as number;
  }
  set subSeccion(value: number) {
    this.setPropertyValue("subSeccion", value);
  }
  ...
  set cellSpacing(value: number) {
    if (value < 0 || value > 264.6) {
      throw new RangeError(
        "The measurement must be between 0 px and 264.6 px."
      );
    }
  ...
  public getPropertyValue(property: string): Object {
    ...
    if (hasValue) {
      ...
      if (
        !isNullOrUndefined(this.uniqueTableFormat) &&
        this.uniqueTableFormat.propertiesHash.containsKey(propertyType)
      ) {
  ...
  private setPropertyValue(property: string, value: Object): void {
    ...
    if (isNullOrUndefined(this.uniqueTableFormat)) {
      ...
      let propertyType: number = WUniqueFormat.getPropertyType(
        this.uniqueTableFormat.uniqueFormatType,
        property
      );
      if (
        this.uniqueTableFormat.propertiesHash.containsKey(propertyType) &&
      ...
      this.uniqueTableFormat = WTableFormat.uniqueTableFormats.updateUniqueFormat(
  ...
  private initializeUniqueTableFormat(
    ...
    let uniqueTableFormatTemp: Dictionary<number, object> = new Dictionary<
      number,
      object
    >();
    this.addUniqueTableFormat(
      "subSeccion",
      property,
      propValue,
      uniqueTableFormatTemp
    );
    this.addUniqueTableFormat(
      "allowAutoFit",
      property,
    ...
    this.uniqueTableFormat = WTableFormat.uniqueTableFormats.addUniqueFormat(
      uniqueTableFormatTemp,
      WTableFormat.uniqueFormatType
    );
  }
  ...
  private addUniqueTableFormat(
    ...
    propValue: Object,
    uniqueTableFormatTemp: Dictionary<number, object>
  ): void {
  ...
  public cloneFormat(): WTableFormat {
    ...
    tableFormat.subSeccion = this.subSeccion;
    ...
    tableFormat.borders = isNullOrUndefined(this.borders)
      ? undefined
      : this.borders.cloneFormat();
    tableFormat.shading = isNullOrUndefined(this.shading)
      ? undefined
      : this.shading.cloneFormat();
  ...
  public copyFormat(format: WTableFormat): void {
    if (!isNullOrUndefined(format)) {
      if (!isNullOrUndefined(format.uniqueTableFormat)) {
        this.subSeccion = format.subSeccion;
